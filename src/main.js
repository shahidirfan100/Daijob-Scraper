            ...
            async requestHandler({ request, $, enqueueLinks, log: crawlerLog }) {
                ...
                if (label === 'LIST') {
                    ...
                    if (collectDetails) {
                        ...
                        if (toPush.length) { await Dataset.pushData(toPush.map(u => ({ url: u, _source: 'remote.co' }))); saved += toPush.length; }
                    }
                ...
                if (label === 'DETAIL') {
                    ...
                    try {
                        ...
                        if (!data.company) data.company = $('[class*="company"], .company, .employer').first().text().trim() || null;
                        if (!data.description_html) { const desc = $('[class*="job-description"], .job-description, .description, .entry-content').first(); data.description_html = desc && desc.length ? String(desc.html()).trim() : null; }
                        data.description_text = data.description_html ? cleanText(data.description_html) : null;
                        if (!data.location) data.location = $('[class*="location"], .location').first().text().trim() || null;
